...
async function getCategoryPosts(categorySlug: string, page: number = 0): Promise<{ posts: any[], total: number, category: any }> {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { category } = await getCategoryPosts(slug, 0)

  const categoryTitle = category?.title || slug.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())
...
export default async function CategoryPage({ params, searchParams }: PageProps) {
  ...
  const { posts, total, category } = await getCategoryPosts(slug, currentPage)
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 bg-brand-500/20 border border-brand-400/30 rounded-full px-4 py-2 mb-4">
                ...
                <span className="text-sm font-medium text-brand-300">Category</span>
              </div>

              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
                <span className="text-brand-400">{categoryTitle}</span> Articles
              </h1>
          ...
          <div className="container mx-auto px-4">
            {posts.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 mb-6">No articles in this category yet.</p>
                <Link
                  href="/blog"
              ...
              <>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-12">
                    {displayPage > 1 && (
                      <Link
                        href={displayPage === 2 ? `/blog/category/${slug}` : `/blog/category/${slug}?page=${displayPage - 1}`}
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                        ...
                        return (
                          <Link
                            key={pageNum}
                            href={pageNum === 1 ? `/blog/category/${slug}` : `/blog/category/${slug}?page=${pageNum}`}
                    ...
                    {displayPage < totalPages && (
                      <Link
                        href={`/blog/category/${slug}?page=${displayPage + 1}`}
                        className="inline-flex items-center gap-1 px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors"
                      >
                        Next
                        <ChevronRight className="w-4 h-4" />
